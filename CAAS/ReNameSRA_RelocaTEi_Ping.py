...
def runjob(script, lines):
    cmd = 'perl /rhome/cjinfeng/BigData/software/bin/qsub-pbs.pl --maxjob 60 --lines %s --interval 120 --resource walltime=100:00:00,nodes=1:ppn=6,mem=10G --convert no %s' %(lines, script)
    #print cmd 
    os.system(cmd)
...
def main():
    ...
    if not args.repeat:
        args.repeat = '/rhome/cjinfeng/BigData/00.RD/RelocaTE_i/Simulation/Reference/pong.fa'
    ...
    for read_dir in sorted(read_dirs):
        strain = os.path.split(read_dir)[1]
        ...
        #if 1:
            relocaTE = '%s --te_fasta %s --genome_fasta %s --fq_dir %s --outdir %s --reference_ins %s --sample %s --size 500 --step 1234567 --mismatch 0 --run --cpu 6 --aligner blat --verbose 3 --len_cut_match 20 --len_cut_trim 20 --split' %(RelocaTE, Repeat, Reference, read_dir, outdir, existingTE, strain)
