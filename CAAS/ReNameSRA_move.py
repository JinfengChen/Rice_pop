...
def main():
    ...
    for sra in sras:
        ...
        subdir = './fastq/%s' %(acc2strain[acc][0])
        ...
        if os.path.isfile('%s/%s_%s_2.fastq' %(outdir, acc, acc2strain[acc][0])):
            os.system('mv %s/%s_%s_*.fastq %s' %(outdir, acc, acc2strain[acc][0], subdir))
        #cmd    = []
        #if not os.path.isfile(fq1) and not os.path.isfile(fq1gz):
        #    cmd.append('/opt/sratoolkit/2.1.16/bin/fastq-dump --outdir %s/%s --split-files %s' %(outdir, subdir, sra_cp))
