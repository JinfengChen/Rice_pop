...
#Taxa	Ping_Mean	Copy_estimate	Genome_Depth
#IRIS_313-15896	0	0.0	8.95663397005679
def read_table_depth(infile):
    ...
    with open (infile, 'r') as filehd:
        for line in filehd:
            ...
            if len(line) > 2 and not line.startswith(r'Taxa'): 
                unit = re.split(r'\t',line)
                data[unit[0]] = '%s\t%s\t%s' %(unit[1], unit[2], unit[3])
    return data
...
def main():
    ...
    depth     = read_table_depth('Rice3k_3000_RelocaTEi_mPing_NM2.mPing_copy.depth.txt')
    print "Taxa\tmPing\tPing\tPong\tmPing_depth\tmPing_Copy_Estimate\tGenome_Depth"
