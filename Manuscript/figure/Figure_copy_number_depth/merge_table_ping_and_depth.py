...
def main():
    ...
    print >> ofile, 'Taxa\t%s\t%s_copy_estimate\tGenome_Depth' %(copy['Taxa'], copy['Taxa'])
    for strain in sorted(copy.keys()):
        if depth.has_key(strain):
            print >> ofile, '%s\t%s\t%s\t%s' %(strain, copy[strain], float(copy[strain])/float(depth[strain]), depth[strain])
    ofile.close()
