...
def convert_name(fastafile):
    ...
    newfile = re.sub(r'.Nclean', r'.Nclean.name.fa', fastafile)
    ofile = open(newfile, 'w')
    #old = defaultdict(lambda : int())
    #new = defaultdict(lambda : int())
    for record in SeqIO.parse(fastafile,"fasta"):
        m = s.search(str(record.description))
        if not m:
...
def clean_mPing_element(indir, output):
    ...
    if not os.path.exists('%s.mPing.clean.Nclean.mPing_var.msa.vcf' %(output)):
                #cmd = []
                ...
                os.system('/opt/linux/centos/7.x/x86_64/pkgs/ncbi-blast/2.2.26/bin/blastall -i %s.mPing.clean.Nclean.name.fa -d %s.mPing.clean.Nclean.name.fa -o %s.mPing.clean.Nclean.name.blast' %(output, output, output))
                ...
                os.system('cat %s.mPing.clean.Nclean.name.blast.table.representive.fa mPing_variants.fa > %s.mPing.clean.Nclean.name.blast.table.representive.mPing_var.fa')
                os.system('/opt/linux/centos/7.x/x86_64/pkgs/muscle/3.8.425/bin/muscle -in %s.mPing.clean.Nclean.name.blast.table.representive.mPing_var.fa -out %s.mPing.clean.Nclean.name.blast.table.representive.mPing_var.fa' %(output, output))
                os.system('/opt/linux/centos/7.x/x86_64/pkgs/java/jdk1.8.0_45/bin/java -jar /rhome/cjinfeng/BigData/software/jvarkit/dist/msa2vcf.jar -a %s.mPing.clean.Nclean.name.blast.table.representive.mPing_var.msa > %s.mPing.clean.Nclean.name.blast.table.representive.mPing_var.vcf' %(output, output))
...
def main():
    ...
    final_clean_mPing_fa = clean_mPing_element(args.input, args.output)
    #final_clean_mPing_fa = ['rice3k.6.mPing.clean.Nclean.fa']
    summary_mPing_assembly(final_clean_mPing_fa, '%s.clean_flank' %(args.output))
    summary_mPing_assembly(final_clean_mPing_fa, '%s.clean_N' %(args.output))
