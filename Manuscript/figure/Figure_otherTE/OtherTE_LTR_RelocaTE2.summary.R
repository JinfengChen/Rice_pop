...
x = read.table(file="OtherTE_LTR_RelocaTE2.summary.list", sep='\t', header=1)
y1 = setNames(x[,7], x[,1])
...
pdf("OtherTE_LTR_RelocaTE2.summary.pdf", width=10, height=9)
...
#5
par(mar=c(4,0.5, 2, 1)) #set left and right to be tight with other plot
barplot(y5,horiz=TRUE,width=1,space=0, xlim=c(0, 240),  ylim=c(0.5,length(tree$tip.label)),names="", axes=FALSE)
axis(1, at= c(0, 80, 160, 240), line=0)
mtext("RIRE3", side=1,font=3, at=120,line=2.5, cex=1, col="black")
#6
par(mar=c(4,0.5, 2, 1)) #set left and right to be tight with other plot
