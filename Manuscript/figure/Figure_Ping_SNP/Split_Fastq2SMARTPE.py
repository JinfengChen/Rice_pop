...
def split_fastq_seq(fastqfile):
    ...
    for read in sorted(data.keys()):
        if data[read].has_key(1) and data[read].has_key(2):
            ...
            SeqIO.write(data[read][1], ofile_smart, 'fastq')
            SeqIO.write(data[read][2], ofile_smart, 'fastq')
