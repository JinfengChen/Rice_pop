...
#ERS467756.NM2	57	0	10	0
def read_table_ping_SNP(infile):
    data = defaultdict(lambda : str())
    with open (infile, 'r') as filehd:
        for line in filehd:
            line = line.rstrip()
            if len(line) > 2: 
                unit = re.split(r'\t',line)
                strain = re.sub(r'.NM2', r'', unit[0])
                data[strain] = '\t'.join(unit[1:])
    return data
...
def main():
    ...
    if not args.output:
        args.output = '%s.acc2name.txt' %(args.input)
 
    acc2name = read_download_table('rice_line_3000.download.list')
    #depth = read_table_depth(args.depth)
    ping  = read_table_ping_SNP(args.input)
    ofile = open(args.output, 'w')
    for strain in sorted(ping.keys()):
        print >> ofile, '%s\t%s' %(acc2name[strain], ping[strain])
