...
#                unit = re.split(r'\t',line)
#                summary[0] += 1
#                if 3k_anno.has_key(unit[0]):
#                    summary[1] += 1
#                    data[unit[0]] = 3k_anno[unit[0]]
#    print 'input strains: %s' %(str(summary[0]))
#    print 'output strains: %s' %(str(summary[1]))
#    return data

#def sum_anno(anno, pop):
...
def main():
    ...
    3k_anno = read_anno('rice_line_ALL_3000.anno.landrace.list')
    #tt_anno    = sub_anno(3k_anno, args.input)
    #3k_anno_sum  = sum_anno(3k_anno)
    #tt_anno_sum  = sum_anno(tt_anno)
    #ofile = open('%s.summary_pop.txt' %(args.input), 'w')
    #print >> ofile, 'Population\t3k_rice\tSubset'
    #for p in sorted(pop):
    #    print >> ofile, '%s\t%s\t%s' %(p, 3k_anno_sum[p], tt_anno_sum[p]) 
    #ofile.close()
