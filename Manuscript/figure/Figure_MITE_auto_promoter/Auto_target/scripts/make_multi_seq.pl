...
# ./make_multi_seq.pl db dbout.clstr multi-seq 1
...
while($ll = <TMP>) {
  if ($ll =~ /^>/) {
    if ($size_cutoff == 1 and $no == 1){
      foreach $i (@ids) { $id2cid{$i} = $cid;}
    }elsif($size_cutoff > 1 and $no >= $size_cutoff) {
      foreach $i (@ids) { $id2cid{$i} = $cid;}
    }
...
while($ll=<FASTA>) {
  if ($ll=~ /^>(\S+)/) {
    ...
    if (defined($cid)) {
      ...
      open(OUT, ">> $out_dir/$cid.fa") || die "can not open file to write $out_dir/$cid.fa";
      $outfile_open = 1;
      $flag = 1;
    }
    else {
