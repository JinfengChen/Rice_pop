...
def main():
    ...
    print >> ofile, 'Target_Dirs\tNumberOfSequence\tTop_Identity:Top_Seq_Number\tTop_Align_Len:Top_Identity:Top_Seq_Number' 
    for d in sorted(query_dirs):
        ...
        if not os.path.exists(blast_table) and os.path.exists(seq) and os.path.exists(blast):
            ...
            #print top_id
            #print top_id[1][0]
            print >> ofile, '%s\t%s\t%s\t%s:%s\t%s:%s\t%s\t%s' %(d, query_name, seq_n, top_id[0][0], top_id[0][1], top_id_len[0][0], top_id_len[0][1], '\t'.join(re.split(r':', top_id_len[0][0])), top_id_len[0][1])
