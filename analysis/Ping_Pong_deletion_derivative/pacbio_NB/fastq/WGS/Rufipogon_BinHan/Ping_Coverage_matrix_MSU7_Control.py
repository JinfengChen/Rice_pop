    ...
    with open (infile, 'r') as filehd:
        for line in filehd:
            ...
            if len(line) > 2:
                ...
                if unit[0] == 'Chr6' and int(unit[1]) >= 24310420 and int(unit[1]) <= 24317420:
                    if int(unit[2]) > 0:
                        data.append('10')
                    else:
                        data.append('0')
...
def main():
    ...
    ofile = open('control_coverage.sh', 'w')
    count = 0
    for bam in sorted(bams):
        print bam
        coverage  = '%s.pong.coverage.txt' %(bam)
        coverage1 = '%s.control.coverage.clean.txt' %(bam)
        if not os.path.exists(coverage1) or int(os.path.getsize(coverage1)) == 0:
            print coverage
            #cmd = '%s view -b %s Chr6 | %s genomecov -ibam stdin -d > %s' %(samtools, os.path.abspath(bam), bedtools, os.path.abspath(coverage))
            sed = 'awk -F"\\t" \'$2>24310420 && $2<24317420\' %s > %s' %(os.path.abspath(coverage), os.path.abspath(coverage1))
            #print >> ofile, cmd
            print >> ofile, sed
            count += 1
    ofile.close()
    if count > 0:
        runjob('control_coverage.sh', 4)
    #####summarize coverage into matrix
    covs = glob.glob('%s/*.control.coverage.clean.txt' %(args.input))
