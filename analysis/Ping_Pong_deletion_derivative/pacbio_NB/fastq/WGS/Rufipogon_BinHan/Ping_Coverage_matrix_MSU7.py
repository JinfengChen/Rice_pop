    ...
    with open (infile, 'r') as filehd:
        for line in filehd:
            ...
            if len(line) > 2:
                ...
                if unit[0] == 'Chr6' and int(unit[1]) >= 23520641 and int(unit[1]) <= 23527981:
                    if int(unit[2]) > 0:
                        data.append('10')
                    else:
                        data.append('0')
...
def main():
    ...
    for bam in sorted(bams):
        ...
        coverage1 = '%s.ping.coverage.clean.txt' %(bam)
        if not os.path.exists(coverage1) or int(os.path.getsize(coverage1)) == 0:
            ...
            sed = 'awk -F"\\t" \'$2>23520641 && $2<23527981\' %s > %s' %(os.path.abspath(coverage), os.path.abspath(coverage1))
            #print >> ofile, cmd
            print >> ofile, sed
            count += 1
    ofile.close()
    if count > 0:
        runjob('ping_coverage.sh', 4)
    #####summarize coverage into matrix
    covs = glob.glob('%s/*.ping.coverage.clean.txt' %(args.input))
