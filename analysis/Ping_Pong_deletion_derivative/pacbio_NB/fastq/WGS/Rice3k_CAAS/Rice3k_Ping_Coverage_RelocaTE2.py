    ...
    with open (infile, 'r') as filehd:
        for line in filehd:
            ...
            if len(line) > 2:
                ...
                if unit[0] == 'ping' and int(unit[1]) >= 1 and int(unit[1]) <= 5341:
                    if int(unit[2]) > 0:
                        data.append('10')
                    else:
                        data.append('0')
...
def ping_coverage_3k(infile):
    ...
    bedping  = '/rhome/cjinfeng/Rice/Rice_population_sequence/Rice_3000/analysis/Ping_Pong_deletion_derivative/pacbio_NB/fastq/WGS/Rice3k_CAAS/genome.ping_element.bed'
    with open (infile, 'r') as filehd:
        for line in filehd:
            line = line.rstrip()
            if len(line) > 2 and not line.startswith(r'Taxa'):
                unit = re.split(r'\t',line)
                acc = acc2name[unit[0]]
                bam = '/rhome/cjinfeng/Rice/Rice_population_sequence/Rice_3000/Manuscript/figure/Figure_Ping_SNP/Rice3k_3000_RelocaTEi_Ping_NM2/{}.NM2.bam'.format(acc)
                #bam = 'http://s3.amazonaws.com/3kricegenome/Nipponbare/%s.realigned.bam' %(acc)
                coverage  = 'ping_coverage_3k/%s.ping.coverage.txt' %(unit[0])
                coveragez = 'ping_coverage_3k/%s.ping.coverage.txt.gz' %(unit[0])
                coverage1 = 'ping_coverage_3k/%s.ping.coverage.clean.txt' %(unit[0])
