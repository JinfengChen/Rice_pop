    ...
    with open (infile, 'r') as filehd:
        for line in filehd:
            ...
            if len(line) > 2:
                ...
                #$2>24310420 && $2<24317420
                if unit[0] == 'chr06' and int(unit[1]) >= 24310420 and int(unit[1]) <= 24317420:
                    if int(unit[2]) > 0:
                        data.append('10')
                    else:
                        data.append('0')
    ...
    with open (infile, 'r') as filehd:
        for line in filehd:
            ...
            if len(line) > 2 and not line.startswith(r'Taxa'):
                ...
                coverage1 = 'ping_coverage_3k/%s.control.coverage.clean.txt' %(acc)
                if not os.path.exists(coverage1) or int(os.path.getsize(coverage1)) == 0:
                #if os.path.exists(local_bam) and not os.path.exists(coverage1):
                    ...
                    #cmd = '%s view -b %s chr06 | %s genomecov -ibam stdin -d > %s' %(samtools, os.path.abspath(local_bam), bedtools, os.path.abspath(coverage))
                    #gz  = '/usr/bin/pigz %s -p 1 -f' %(os.path.abspath(coverage))
                    sed = 'zcat %s | awk -F"\\t" \'$2>24310420 && $2<24317420\' > %s' %(os.path.abspath(coveragez), os.path.abspath(coverage1))
                    #print >> ofile, down
                    #print >> ofile, index
                    #print >> ofile, cmd
                    #print >> ofile, gz
