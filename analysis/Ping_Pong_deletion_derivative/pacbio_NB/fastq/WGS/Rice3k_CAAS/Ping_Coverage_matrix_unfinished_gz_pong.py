...
def usage():
    test="name"
    message='''
python Ping_Coverage_matrix_unfinished_gz.py --input ping_coverage_3k
...
def main():
    ...
    bams  = glob.glob('%s/*.ping.coverage.txt.gz' %(args.input))
    #####generate coverage file for bams
    for bam in sorted(bams):
        #print bam
        #coverage = '%s.ping.coverage.txt' %(bam)
        coverage = bam
        #clean    = '%s.clean.txt' %(os.path.splitext(os.path.splitext(bam)[0])[0])
        clean    = re.sub(r'ping.coverage.txt.gz', r'pong.coverage.clean.txt', coverage)
