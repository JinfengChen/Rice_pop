    ...
    with open (infile, 'r') as filehd:
        for line in filehd:
            ...
            if len(line) > 2:
                ...
                if unit[0] == 'chr03' and int(unit[1]) >= 29071995 and int(unit[1]) <= 29077870:
                    if int(unit[2]) > 0:
                        data.append('10')
                    else:
                        data.append('0')
        ...
        for line in filehd:
            ...
            if len(line) > 2 and not line.startswith(r'Taxa'):
                ...
                if not os.path.exists(coverage1) or int(os.path.getsize(coverage1)) == 0:
                    cmd = '%s view -b %s chr03 | %s genomecov -ibam stdin -d > %s' %(samtools, bam, bedtools, os.path.abspath(coverage))
...
def main():
    ...
    covs = glob.glob('%s/*.actin.coverage.clean.txt' %('ping_coverage_3k'))
