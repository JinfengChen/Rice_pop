    ...
    with open (infile, 'r') as filehd:
        for line in filehd:
            ...
            if len(line) > 2 and not line.startswith(r'Taxa'):
                ...
                if not os.path.exists(stats) or int(os.path.getsize(stats))==0:
                    cmd = '%s stats %s > %s' %(samtools, bam, os.path.abspath(stats))
                    if os.path.exists(local_bam):
                        cmd = '%s stats %s > %s' %(samtools, local_bam, os.path.abspath(stats))
                    print >> ofile, cmd
                    count += 1
    ofile.close()
    #if count > 0:
    #    runjob('rice_3k_depth.sh', 1)
