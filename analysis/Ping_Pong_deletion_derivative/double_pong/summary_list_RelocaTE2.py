...
def read_down(infile):
    ...
    with open (infile, 'r') as filehd:
        for line in filehd:
            ...
            if len(line) > 2: 
                unit = re.split(r'\t',line)
                data[unit[4]] = [unit[1], unit[3]]
    return data



def sum_info(infile, rice_group, rice_down):
    data = defaultdict(lambda : int())
    ofile = open('%s.list' %(infile), 'w')
    with open (infile, 'r') as filehd:
        for line in filehd:
            line = line.rstrip()
            if len(line) > 2: 
                unit = re.split(r'\t',line)
                data[rice_down[unit[0]][1]] += 1
                print >> ofile, re.sub(r'IRIS', r'IRIS_', rice_down[unit[0]][0])
    ofile.close()
...
def main():
    ...
    rice_info = read_info('rice_line_ALL_3000.anno.list')
    rice_down = read_down('rice_line_3000.download.list')
    sum_info(args.input, rice_info, rice_down)
