...
def main():
    ...
    for fq1 in sorted(strain_fq1):
        ...
        cmd = []
        ##
        cmd.append('''/opt/linux/centos/7.x/x86_64/pkgs/bwa/0.7.12/bin/bwa mem -t 2 %s %s %s | %s view -F4 -bS -o %s.raw.bam -''' %(ping, fq1, fq2, samtools, strain))
    ...
    runjob('run_mapping.sh', 5, 2, 'batch')
