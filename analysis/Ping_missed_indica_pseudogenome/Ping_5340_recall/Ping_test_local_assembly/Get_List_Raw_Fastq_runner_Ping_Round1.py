...
def main():
    ...
    for f in file_list:
        ...
        if not os.path.exists('%s.assembly' %(prefix)):
            ...
            cmd.append('/opt/linux/centos/7.x/x86_64/pkgs/samtools/1.6/bin/samtools view %s Ping_up:50-420 | cut -f1 > %s.upreads.list' %(f, prefix))
            ...
            cmd.append('/opt/linux/centos/7.x/x86_64/pkgs/samtools/1.6/bin/samtools view %s Ping_up:5790-6150 | cut -f1 >> %s.downreads.list' %(f, prefix))
