...
def bamcheck(bam, mping, bamck_file, ril):
    ...
    for line in lines:
        ...
        for i in range(1, len(matches)):
            ...
            if matches[i][1] == matches[i-1][1]:
                matches[i-1][0] = int(matches[i][0]) + int(matches[i-1][0])
                del matches[i]
                j = j + 1
    ...
    #elif int(covered) >= 3 and int(clipped) >= 3:
    #    print >> ofile, 5
    #    return 5
    elif int(covered) >= 3:
        print >> ofile, 0
...
def bamcheck_simple_cover(bam, mping, bamck_file):
    reg = re.compile(r'(.*):(\d+)\-(\d+)')
    match = reg.search(mping)
    start = int(match.groups(0)[1])
    end   = int(match.groups(0)[2])
