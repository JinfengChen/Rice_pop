...
def runjob(script, lines):
    cmd = 'perl /rhome/cjinfeng/BigData/software/bin/qsub-slurm.pl --maxjob 30 --lines %s --interval 120 --task 16 --mem 30G --time 10:00:00 --convert no %s' %(lines, script)
    #print cmd 
    os.system(cmd)
...
def main():
    ...
    for sra in sras:
        ...
        if not os.path.isfile(fq1) and not os.path.isfile(fq1gz):
            cmd.append('/opt/linux/centos/7.x/x86_64/pkgs/sratoolkit/2.5.0/bin/fastq-dump --outdir %s --split-files %s' %(dirname, os.path.abspath(sra)))
            cmd.append('pigz -p 16 %s' %(fq1))
            cmd.append('pigz -p 16 %s' %(fq2))
            #pass
