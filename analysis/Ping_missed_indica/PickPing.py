...
def gff_parse(infile, flank_inf, support_inf):
    ...
    r = re.compile(r'\=')
    ping_gff = re.sub(r'.gff', r'.Ping.raw.gff', infile)
    ping_log = re.sub(r'.gff', r'.Ping.raw.inf', infile)
    #print infile
    #print ping_gff
            ...
            for i in [1, 2]:
                ...
                if not flank_inf[read][str(i)]['pos'] == '':
                    ...
                    if flank_inf[read][str(i)]['pos'] == 'middle':
                        ...
                        #     print >> ofile_log, 'Midlle reads: %s' %(read)
                        #flank = 5
                        if int(flank_inf[read][str(i)]['start']) >= start_int+flank and int(flank_inf[read][str(i)]['start']) <= end_int-flank:
                             data[repid][read] = 1
                             print >> ofile_log, 'Midlle reads: %s' %(read)
...
def parse_blatout(blat_file, flank_inf):
    r        = re.compile(r'.*\/(.*)\_(\d+)\.te\_repeat\.blatout')
    lib_n, lib_t = ['NA','0']
...
def parse_flanking_fastq(fastq_file, flank_inf):
    ...
    r        = re.compile(r'.*\/(.*)\_(\d+)\.te\_repeat\.flankingReads\.fq')
    lib_n, lib_t = ['NA','0']
