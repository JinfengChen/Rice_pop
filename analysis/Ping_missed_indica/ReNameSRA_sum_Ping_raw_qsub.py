...
def runjob(script, lines):
    cmd = 'perl /rhome/cjinfeng/BigData/software/bin/qsub-slurm.pl --maxjob 30 --lines %s --interval 120 --task 1 --mem 5G --time 10:00:00 --convert no %s' %(lines, script)
    #print cmd 
    os.system(cmd)
...
def main():
    ...
    if not args.list:
        args.list = '/rhome/cjinfeng/Rice/Rice_population_sequence/Rice_3000/GigaScience/rice_line_3000.download.list'
    ...
    for call in sorted(os.listdir(args.input)):
        ...
        if args.call == 'RelocaTE':
            ...
            cmd_ping    = 'python %s/PickPing.py --input %s' %(os.path.abspath(os.path.split(args.input)[0]), dirname)
            print >> ofile, cmd_ping
    ofile.close()
    runjob(sh_file, 100)
    ...
    for call in sorted(os.listdir(args.input)):
        ...
        if args.call == 'RelocaTE':
            ...
            ping_nonref_gff  = '%s/repeat/results/ALL.all_nonref_insert.Ping.raw.gff' %(dirname)
