...
def main():
    ...
    popn         = [1, 2, 3, 4, 5]
    pop          = ['Indica', 'Aus/boro', 'Basmati/sadri', 'Temperate japonica', 'Tropical japonica']
    ...
    for i in range(len(strains)):
        ...
        if c_index == 5:
            #Basmati/sadri
            meta[s] = [popn[2], pop[2], float(c[c_index]), float(c[0])+float(c[3])+float(c[4])+float(c[7])]
        elif c_index == 1:
            #Temperate japonica
            meta[s] = [popn[3], pop[3], float(c[c_index]), float(c[2])]
        elif c_index == 2:
            #Tropical japonica
            meta[s] = [popn[4], pop[4], float(c[c_index]), float(c[1])]
        elif c_index == 0 or c_index == 3 or c_index == 4 or c_index == 7:
            #Indica
            meta[s] = [popn[0], pop[0], float(c[0])+float(c[3])+float(c[4])+float(c[7]), float(c[6])]
        elif c_index == 6:
            #Aus/boro
            meta[s] = [popn[1], pop[1], float(c[c_index]), float(c[0])+float(c[3])+float(c[4])+float(c[7])]
         
    for s in sorted(meta.keys()):
        print '%s\t%s' %(s, '\t'.join(map(str, meta[s])))
