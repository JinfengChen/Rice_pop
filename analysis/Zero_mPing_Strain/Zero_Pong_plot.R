pdf("Zero_Pong.pdf", width=7, height=4)
par(mar=c(5,5,4,2))
library(reshape)
library(ggplot2)
library(Rmisc)
...
zero_mping = read.table("Zero_Pong_strains.list")
...
mping <- t(mping)
mping_matrix <- as.matrix(mping)
image(1:429, 1:18, mping_matrix, axes=FALSE, xlab = "", ylab = "", col = c("palegoldenrod", "cadetblue3"))
mtext("mPing", side=1, line=3, font=3, cex=1.2)
mtext("Rice strains", side=2, line=3, font=1, cex=1.2)
axis(2, at=c(0.5, 18.5), labels=c(1, 18), cex.lab=1.4, cex.axis=1.4)
...
image(1:5340, 1:18, ping_matrix, axes=FALSE, xlab = "", ylab = "", col = c("palegoldenrod", "cadetblue3"))
...
image(1:5165, 1:18, pong_matrix, axes=FALSE, xlab = "", ylab = "", col = c("palegoldenrod", "cadetblue3"))
...
image(1:5875, 1:18, control_matrix, axes=FALSE, xlab = "", ylab = "", col = c("palegoldenrod", "cadetblue3"))
