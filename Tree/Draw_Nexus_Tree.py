...
''' %(newick, anno, col, prefix, tip)
    ...
    if int(color) > 0 and ',' in col:
        col_list = re.split(r',', col)
        if len(col_list) == 2:
            R_cmd='''
library("ape")
tree = read.tree(file="%s")
...
plot(tree, edge.color=edge_colors, show.tip.label = %s, cex = 0.5, underscore=TRUE)
...
dev.off()
''' %(newick, anno, col_list[0], col_list[1], color, prefix, tip)
        elif len(col_list) == 3:
                    R_cmd='''
library("ape")
tree = read.tree(file="%s")
...
dev.off()
''' %(newick, anno, col_list[0], col_list[1], col_list[2], color, prefix, tip)
...
#barplot(y,horiz=TRUE,width=1,space=0, xlim=c(-10, 200),  ylim=c(0.5,length(tree$tip.label)),names="", axes=FALSE)
#axis(1, at= c(0, 100, 200), line=1)
barplot(y,horiz=TRUE,width=1,space=0, xlim=c(0, 600),  ylim=c(0.5,length(tree$tip.label)),names="", axes=FALSE)
axis(1, at= c(0, 300, 600), line=0)
mtext("mPing", side=1,font=3, at=300,line=2.5, cex=1, col="black")
dev.off()
...
def main():
    ...
    if args.subsample:
        ...
        write_R(sub_tree_file, sub_anno_file, column, color, sub_prefix, args.showtips)
    elif args.sublist:
        if not args.subtitle:
            args.subtitle = 'CAAS'
        sub_tree_file = '%s.%s.tree' %(tree_file, args.subtitle)
        ...
        write_R(tree_file, anno_file, column, color, prefix, args.showtips)    
