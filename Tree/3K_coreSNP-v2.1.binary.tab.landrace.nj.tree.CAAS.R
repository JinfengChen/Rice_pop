...
library("ape")
tree = read.tree(file="3K_coreSNP-v2.1.binary.tab.landrace.nj.tree.CAAS.tree")
x = read.table(file="3K_coreSNP-v2.1.binary.tab.landrace.nj.tree.CAAS.anno", sep='\t', header=1)
y = setNames(x[,7], x[,1])
...
pdf("3K_coreSNP-v2.1.binary.tab.landrace.nj.tree.CAAS.pdf")
